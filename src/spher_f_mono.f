...
C  Modification for GAMMA DISTRIBUTION, aa (eff radius) and bb (eff variance)    

                                                                     

C  Last modified 11/20/2000

C  Gamma distribution modification for aa and bb parameters                                            
...
C  available in the .pdf and .ps formats at 
...
C     Try to read parameters from file first
      OPEN(UNIT=99, FILE='params.dat', STATUS='OLD', ERR=100)
      READ(99,*,ERR=100,END=100) LAM, MRR, MRI, AA, BB
      CLOSE(99)
      WRITE(*,*) 'Parameters read from params.dat:'
      GO TO 200

C     If file doesn't exist, try command line arguments
100   CONTINUE
      NARGS = IARGC()
      IF (NARGS .GE. 5) THEN
         CALL GETARG(1, CARG)
         ...
         CALL GETARG(5, CARG)
         read(CARG,*) BB
         WRITE(*,*) 'Parameters from command line:'
      ELSE
         WRITE(*,*) 'Using default parameters:'
      ENDIF

200   CONTINUE
      WRITE(*,*) 'Wavelength=',LAM,' (um)'
      WRITE(*,*) 'Refractive index=',MRR,'+i*',MRI
      WRITE(*,*) 'Effective radius (aa)=',AA,' (um)'
      WRITE(*,*) 'Effective variance (bb)=',BB
    
C     BB is now passed as input parameter
...
C*********** (also see NPNA in SUBROUTINE MATR) ***********
...
C        180*(I-1)/(NPNA-1) (DEGREES), WHERE I NUMBERS THE ANGLES
      ...
      PARAMETER (NMIE=10000, NPL=2*NMIE, NPNA=18000)
